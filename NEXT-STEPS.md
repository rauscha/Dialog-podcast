...
- [ ] **Phase 3 — multi-feed publishing. ← START HERE (split into 3 feeds before scaling).** `update_rss` gains `feed_meta`; per-show channel metadata + cover art (`assets/cover-{mfm,fetal,ai}.jpg`); thread feed filename into `git_publish`; `run_digest` passes `feed_meta`; `ledger.record_episode` after a successful publish (this is where the ledger/backlog starts filling).
