...
- [ ] **Activate the Task Scheduler entry.** Run `.\register_scheduled_task.ps1` as Administrator once. Test with `Start-ScheduledTask -TaskName "Dialog-podcast-daily-digests"`. Restart Telegram bot so `/digest` commands are live.
- [ ] **Submit Fetal + AI feeds to Spotify for Creators.** URLs: `https://rauscha.github.io/Dialog-podcast/feed-fetal.xml` and `https://rauscha.github.io/Dialog-podcast/feed-ai.xml`.
