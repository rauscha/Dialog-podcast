...
- [x] **Phase A — audio-engineering finish — DONE (2026-06-06).** A1+A2 (`e8ef64d`): final master → two-pass linear loudnorm, retarget −14 LUFS/−1.0 dBTP, + de-esser (chain highpass→lowpass→deesser→loudnorm, tuned `deesser i=0.40`). A4 (`49e859b`): per-clip two-pass loudnorm to the program target in `clip_mixer.py`. A3 (`c33970c`): micro-crossfades at concat joins (10ms speech / 2.5s qsin music bookends), argv-safe + clamped + hard-concat fallback. New shared `audio_utils.py` (`two_pass_loudnorm`, `deesser_filter`) de-dupes the loudnorm code across per-turn / master / clips. **Verified in isolation** (master −14.3 LUFS/−1.2 dBTP; clips 23 dB apart both → −13.8; crossfade math exact; 200-seg scale 0.4s). **A5 (48k) deliberately deferred** — nice-to-have, 44.1k is fine for speech. **VERIFIED on a live render (2026-06-07, "The Last Problems Before Mars", `21b09a3`):** closing callback + sonic-footnote Phase 1.5 both fired; NASA cue drew (`nasa_mars_wind`, after turn 31); music+speech crossfades + de-ess (0.40) ran. **Bug caught + fixed (`ca1c665`):** `config.json` still pinned the pre-Phase-A `-16/-1.5`, which overrides DEFAULTS in the priority chain, so real renders silently ignored the `-14/-1.0` retarget (isolation tests passed only because they called `two_pass_loudnorm` with explicit `-14` args). The interplanetary episode published at `-16.3 LUFS / -1.7 dBTP` (left as-is — valid podcast spec); config now corrected so the *next* render hits `-14`. **CONFIRMED on a live render (2026-06-08, `5243051`): -14.3 LUFS / -1.2 dBTP** — the feared undershoot to ~-15 did not happen; linear loudnorm lands essentially on the -14 target without a limiter.
- [ ] **Phase B** — ~~emotion threading (B1)~~ **DONE (2026-06-08, `aaa7925`)**: `_build_tts_instructions` takes an optional `prev_emotion_tag`; `_tts_two_host` threads the previous non-empty turn's tag (computed in order, before the thread pool) so prosody flows across the cut instead of resetting to neutral. Clause only fires when the prior turn had a genuine, differing tag; default/identical tags and the fallback call sites add nothing. Verified by branch test + compile. **Live-rendered + published 2026-06-08 (`5243051`, "The Chip Shortage That Ended Budget Gaming", 8:45) across all 78 turns** — awaiting the user's ear-verdict on whether prosody now flows across cuts. → ~~edge-trim + variable gaps (B2)~~ **DONE (2026-06-08, `566cb3a`)**: single spacing ruler — `_edge_trim_silence` strips TTS-baked head/tail silence to a 40ms pad (conservative -50dB peak `silenceremove`, best-effort, runs after loudnorm); `_inter_turn_gap_ms` flexes the gap by context (latch 70 > same-speaker 120 > reaction 110 > landing beat 300 > base 180). Assembly interleaves per-boundary gap segments (cached by duration); footnote-adjacent boundaries keep base. A3's 10ms crossfade stays a pure click-killer — no double-apply. All flag-gated (`turn_edge_trim`, `turn_variable_gaps`); off ⇒ byte-identical to before. Verified by unit tests (gap logic) + real-audio edge-trim (2.0s→1.08s) + compile/import. **Live-rendered + published 2026-06-08 (`5243051`, same episode as B1)** — awaiting the user's ear-verdict on timing/breath + any clipped onsets from edge-trim. All knobs are config (`turn_gap_*_ms`, `turn_reaction_max_chars`, `turn_beat_min_chars`, `turn_edge_keep_ms`) if tuning is needed. → ~~disfluency/backchannel pass (B3)~~ **DONE (2026-06-08, `ba6479a`)**: new `_DISFLUENCY_SYSTEM` stage between symmetry-break and fact-check (runs before fact-check so corrections still apply to the final wording; non-digest only, same gate as symmetry-break). Sparse, surgical speech-realism layer — (1) soft disfluencies before genuinely hard words, shaped `filler → pause → connector` ("the clever bit is — um, so, it's the memory controller"), placed *before* the important word, never mid-stride, never on short lines; (2) short backchannel turns ("mm-hmm" / "right" / "oh — interesting") as their own lines for the *listening* host. Density-capped at ~1 addition per 6 turns (overdoing it sounds *more* fake). Flag-gated (`use_disfluency_pass`, default on, bool-config); off or digest ⇒ script byte-identical. **Verified on a real LLM call (acceptance = read the diff, not ear):** 8-turn sample drew exactly one disfluency (correct filler→pause→connector shape) + one backchannel as its own turn, facts unchanged, em-dash pauses genuine U+2014 (the `�` in console was just cp1252, not mojibake). **Not yet live-rendered** — needs an episode to exercise it end-to-end + an ear-verdict that the disfluencies read as natural hesitation, not tics. → ~~overlaid backchannels (B4, stretch)~~ **BUILT, DEFAULT OFF (2026-06-08, `fa9f990`)**: folds B3's short listening-host reactions ("mm-hmm", "right") onto the *tail* of the talking host's line as a ducked overlay instead of a sequential segment, so the agreement physically overlaps. `_overlay_backchannel` delays the backchannel to start `backchannel_lead_ms` (120) before the base ends, ducks it `backchannel_duck_db` (8) under, sums with `amix normalize=0` (default halving would gut the main voice), `alimiter ~-1 dBFS` as clip insurance (master re-loudnorms anyway). `_is_backchannel_turn` = different speaker than prior + non-question + ≤`backchannel_max_chars` (20); fold pre-pass runs before the B2 gap interleave, only targets the immediately-prior real turn, never one already overlaid (resume can't stack). Best-effort — any overlay failure leaves the turn sequential, never load-bearing. **`use_overlaid_backchannels=False` by default** (higher-risk mixing; walkthrough says enable only after B1–B3 prove out by ear). Flag off ⇒ fold block skipped, assembly byte-identical to pre-B4. **Verified mechanically only** (sound still needs an ear): detection correct across mm-hmm/long/question/first/same-speaker; real-audio mix lands the overlap exactly where designed (base 2.0s → bc delayed 1.88s → mixed 2.33s), stereo+rate preserved, no clip. **To try it: set `use_overlaid_backchannels=true` and render — then listen for whether the murmur overlaps naturally or muddies the main voice.** **Phase B is now feature-complete (B1–B4); B1/B2 live-published + awaiting ear, B3/B4 await a live render.**
- [~] **Phase C** — anti-slop critic + gate (C1); persona vocabulary + enforced disagreement (C2, small). **First pass shipped (2026-06-17, `a838564`): standalone deterministic `anti_slop.py` linter (report-only).** Gate-vs-warn, threshold, optional LLM critic, and C2 are deferred design calls — see `.handoff/PENDING-DECISIONS.md`.
...
- [x] **Phase 2 — Wikimedia Commons backend (2026-06-17, `4f68d56`).** MediaWiki API search + `extmetadata` license parsing (PD/CC0/CC-BY/BY-SA; NC/ND rejected), topical ranking, per-file attribution. Covers `commons_morse_code`, `commons_metronome`, `commons_tuning_fork`. Verified live against the Commons API.
...
- [x] **BUG A — loudnorm the footnote (2026-06-17, `dc0abf9`).** `_resolve_cue` runs `audio_utils.two_pass_loudnorm` on the trimmed clip to `cfg["audio_loudness_i"]` (mirrors clip_mixer A4). Verified: −52.8 → −14.3 LUFS.
- [x] **BUG C — relevance floor (2026-06-17, `93c0afb`).** `_rank_nasa_results` keeps only results ≥ `sonic_footnote_min_overlap` (topical overlap excl. boilerplate); drops to silence otherwise. Verified.
- [x] **BUG B — guaranteed herald (2026-06-17, `ae34f95`).** `_cue_is_heralded` requires the placement turn (or next) to reference the sound (`sonic_footnote_require_herald`, default on); else drop. Injection deferred (turn-count sync). Verified.
- [x] **BUG D — single arrangement ruler (2026-06-17, `0cd3263`).** Footnote boundaries use one deliberate symmetric pad (`sonic_footnote_pad_ms`, 350); A3 stays the click-killer, B2 gaps don't apply there. Verified.
- **Feature still `use_sonic_footnotes=false`** — re-enable is an ear-check (see `.handoff/PENDING-DECISIONS.md`).
...
- [x] Per-user rate limit in the Telegram bot (2026-06-17, `75a901d`).
...
- [x] MusicGen model cached between intro and outro (2026-06-17, `9af7dcb`) — it was reloading twice/episode; now one load.
- [x] `/queue` ETA display in the bot (2026-06-17, `75a901d`) — per-episode + back-to-back estimate; `/status` est-remaining.
- [x] Website loading state during initial `fetch(feedUrl)` (2026-06-17, `d79fd1a`) — accessible spinner + aria-busy.
